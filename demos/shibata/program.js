...
let catx = #x * 20
let caty = #y * 20
let b = caty / 20 - a * catx / 20
let input = f 0
let isapositive = if a > 0 then 1
  else -1
let isinputpositive = if input > 0 then 1
  else -1
let width = 600
let height = 600
...
let rec make_ladder v8 v10 =
  if v10 = 0 then v8
  else make_ladder (line [(30 * isapositive, 20)] Color.black :: v8) (v10 - 1)
...
let draw_cat {cat = (x4, y5); y = y2_v; down = down2_v} v4 =
  ...
  in (if down2_v then place_image (read_image "https://4.bp.blogspot.com/-BpE6K5Fxoz8/XAY5oynltOI/AAAAAAABQdM/GBzM-1Fa2LwlTF3N0WXbOWYqibxCPnfPwCLcBGAs/s800/cat06_moyou_chatora.png" 120 120) (x4 + 280, 250 - y5) (place_images e (d input) (place_image (polygon [(130, -5); (0, 20); (-130, -5)] (make_color 128 92 52)) (catx + 300, 300 - caty) (place_image (polygon [(130, 0); (-8, 100); (-120, 0)] Color.white) (catx + 300, 280 - caty) v4)))
...
let on_tick {cat = (x2, y2); y = y; down = down} =
  if down then {cat = (x2 - 20 * isapositive, y2 - a * 20 * isapositive); y = y - isapositive; down = down}
  else if y = input then {cat = (x2, y2); y = 10 / a; down = true}
  else {cat = (x2, y2); y = y + isinputpositive; down = down}
...
let draw_last {cat = cat4_v; y = y6_v; down = down6_v} =
  ...
  else place_image (text (if (if 0 < b then input > b
  else input < b) then "通り過ぎた..."
  else "届かなかった...") 20 Color.black) (300, 300) (place_image (text "GAME OVER" 30 Color.red) (300, 250) (draw {cat = cat4_v; y = y6_v; down = down6_v}))
;; big_bang {cat = (catx, caty); y = 0; down = false}
  ...
  ~onload:false;;
*/}.toString().replace("#a", a[i]).replace("#x", x[i]).replace("#y", y[i]).split("\n").slice(1,-1).join("\n");
}
