...
let draw {usagi = usagi_v; kame = kame_v} =
  ...
  else usagi_right) usagi_v (place_image (text "(490, 280)" 25 Color.black) (490, 280) (place_image (text "(210, 350)" 25 Color.black) (210, 350) (place_image (text "(350, 490)" 25 Color.black) (350, 480) (place_image goalzone (350, 525) (place_image back (280, 280) (empty_scene width height)))))))
let move_u = 30
let move_k = 10
let move (x, y) v =
  if y < 280 then (x + v, (x + v) - 140)
  else if x > 140 && y = 280 then (x - v, y)
  else if y > 490 then (x, y)
  else (x + v, x + v + 140)
let move_break (ux, uy) (kx, ky) move_u =
  if uy = 280 && ky <= 350 then (490, 280)
  else if ky > 490 then (ux, uy)
...
let stopdraw {usagi = (ux, uy); kame = (kx, ky)} =
  place_image kame_right (kx, ky)
    (place_image  usagi_right (ux, uy)
       (place_image (text "カメがゴール！ゲーム終了！" 40 Color.cyan) (280, 50)
