...
let sem = text ";" 80 Color.black
let rparen = text "]" 80 Color.black
let lparen = text "[" 80 Color.black
...
let judge_list_im lst h =
  let i = judge_i lst h
  in (if i = 1 then [rparen; lparen]
  else if i = 2 then [rparen; sem; lparen]
  else if i = 3 then [rparen; sem; sem; lparen]
  else [rparen; lparen])
let judge_list_p lst h =
  let i = judge_i lst h
  in (if i = 1 then [(r0, h); (sm1, h)]
  else if i = 2 then [(r0, h); (sm1, h); (sm2, h)]
  else if i = 3 then [(r0, h); (sm1, h); (sm2, h); (l0, h)]
  else [(r0, h); (p1, h)])
