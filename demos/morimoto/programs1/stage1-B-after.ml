...
(*         終了条件（点B上に飛行機が置かれるor外) *)
let on_B {airplane_zahyou = (airplane_x, airplane_y); sun_zahyou = sun_zahyou; lop = lop; airplane_houkou = airplane_houkou} =
...
let draw_last {airplane_zahyou = (airplane_x, airplane_y); sun_zahyou = sun_zahyou; lop = lop; airplane_houkou = airplane_houkou} =
  if airplane_x = 400 && airplane_y = 400 then place_image2 (text "B CLEAR!!" 50 Color.white) (width / 2, height / 2) (place_image2 (rectangle width height (make_color 204 204 204 ~alpha:100)) (width / 2, height / 2) (draw {airplane_zahyou = (airplane_x, airplane_y); sun_zahyou = sun_zahyou; lop = lop; airplane_houkou = airplane_houkou}))
