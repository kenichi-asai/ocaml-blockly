...
function openModal() {
    var element =
	 '<div id="container" class="sample"></div>' + 
...
function closeModal() {
    const workspaceArea = document.getElementById('workspaceArea');
    const blockArea = document.getElementsByClassName('blocklyBlockCanvas')[0];
    if (blockArea.hasChildNodes()) {
...
Typed.setOCamlFileURL = function(url) { // url without ".ml"
  Typed.OCamlFileURL = url;
}

Typed.getCode = function(url) {
  const xmlHttp = new XMLHttpRequest();
...
Typed.programToRun = function () {
  try {
    ...
    var before = '';
    var after = ''; 
    if (Typed.OCamlFileURL !== '') {
      before = Typed.getCode(Typed.OCamlFileURL + "-before.ml");
      after = Typed.getCode(Typed.OCamlFileURL + "-after.ml");
    }
...
Typed.newToplevel = function() {
    ...
    window.open('example.html', '_blank', 'fullscreen = yes');
}
