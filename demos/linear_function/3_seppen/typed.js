...
Typed.selectProgram = function (url) {
    var stage = document.getElementById('program').value;
    document.getElementById('image').src = "stage"+stage+".png";
    document.getElementById('a1').textContent = a[stage-1];
    if (a[stage-1] == 1) document.getElementById('a2').textContent = "";
    else if (a[stage-1] == -1) document.getElementById('a2').textContent = "-";
    else document.getElementById('a2').textContent = a[stage-1];
    document.getElementById('x').textContent = x[stage-1];
    document.getElementById('y').textContent = y[stage-1];
    after = afterary[stage-1];
    Typed.loadCode2();
}
...
Typed.loadCode2 = function() {
    const code = main[document.getElementById('program').value-1];
      if (code) {
        openModal();
	setTimeout(function() {
        BlockOfOCamlUtils.codeToBlock(code);
